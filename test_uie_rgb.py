...
from TartanVO_UIE_rgb import UIETartanVORGB
...
def get_args():
    ...
    parser.add_argument('--uie-model-name', default='',
                        help='name of pretrained UIE model (default: "")')
    ...
    parser.add_argument('--subpipe', action='store_true', default=False,
                        help='subpipe test (default: False)')
    parser.add_argument('--aqualoc', action='store_true', default=False,
                        help='aqualoc test (default: False)')
...
if __name__ == '__main__':
    ...
    testvo = UIETartanVORGB(args.model_name, args.uie_model_name)
    print(testvo.vonet.parameters)
    ...
    if args.kitti:
        ...
        datastr = 'aqualoc'
        #args.image_height = 608
        #args.image_width = 968
    else:
        datastr = 'tartanair'
    ...
    if args.pose_file.endswith('.txt'):
        ...
        elif datastr=='subpipe':
            print("==> ATE: %.4f" %(results['ate_score']))
        elif datastr=='aqualoc':
            print("==> ATE: %.4f" %(results['ate_score']))
        else:
        ...
        plot_traj(results['gt_aligned'], results['est_aligned'], vis=False, savefigname='results/'+testname+'_wflow.png', title='ATE %.4f' %(results['ate_score']))
        np.savetxt('results/'+testname+'_wflow.txt',results['est_aligned'])
    else:
